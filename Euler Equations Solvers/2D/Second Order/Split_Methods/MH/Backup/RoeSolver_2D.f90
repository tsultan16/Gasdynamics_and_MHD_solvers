...
integer,parameter::fluidInit=3 !1:V Shock, 2:Oblique Shock, 3:Riemann Problem File input 4:Kelvin-Helmholtz Instability 
...
real*8::rhot,ut,vt,wt,Ht,at,lambda(5),K1(5),K2(5),K3(5),K4(5)
real*8::K5(5),alpha(5)
...
real*8::fleft(-1:nx+2,-1:ny+2,5),fright(-1:nx+2,-1:ny+2,5)
...
if(debug==1)then
  print*,'Characetristic Speeds(1,3,4)=',lambda(1),lambda(3),lambda(4)
end if
...
if(lambda1L<0. .and. lambda1R>0.)then
  do kk=1,5
    flux(jj,kk)=FL(kk)+(lambda1L*(lambda1R-lambda(1))/(lambda1R-lambda1L))&
            *alpha(1)*K1(kk) 
  end do
else if(lambda5L<0. .and. lambda5R>0.)then
  do kk=1,5
    flux(jj,kk)=FR(kk)-(lambda5R*(lambda(5)-lambda5L)/(lambda5R-lambda5L))&
            *alpha(5)*K5(kk)
  end do
else
  do kk=1,5
    flux(jj,kk)=0.5*(FL(kk)+FR(kk))-0.5*( alpha(1)*abs(lambda(1))*K1(kk)&
          +alpha(2)*abs(lambda(2))*K2(kk)+alpha(3)*abs(lambda(3))*K3(kk)&
          +alpha(4)*abs(lambda(4))*K4(kk)+alpha(5)*abs(lambda(5))*K5(kk) )   
  end do
end if
