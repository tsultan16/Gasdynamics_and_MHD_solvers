...
integer,parameter::fluidInit=3 !1:V Shock, 2:Oblique Riemann Problem, 3:Riemann Problem File input 4:Kelvin-Helmholtz Instability 
...
!--------------------------------------------------
!Limited Slopes: delta_i,j^(x,y)
!--------------------------------------------------
do ii=0,nx+1
 do jj=0,ny+1 
  do kk=1,5
    slopex(ii,jj,kk)=minmod(u1(ii,jj,kk)-u1(ii-1,jj,kk),u1(ii+1,jj,kk)-u1(ii,jj,kk))
    slopey(ii,jj,kk)=minmod(u1(ii,jj,kk)-u1(ii,jj-1,kk),u1(ii,jj+1,kk)-u1(ii,jj,kk))
  end do
 end do
end do

!--------------------------------------------------------
!Boundary Extrapolated Values: uL_i,j^(x,y), uR_i,j^(x,y)
!--------------------------------------------------------
do ii=0,nx+1
 do jj=0,ny+1
  do kk=1,5
    qLx(ii,jj,kk)=u1(ii,jj,kk)-0.5*slopex(ii,jj,kk)
    qRx(ii,jj,kk)=u1(ii,jj,kk)+0.5*slopex(ii,jj,kk)
    qLy(ii,jj,kk)=u1(ii,jj,kk)-0.5*slopey(ii,jj,kk)
    qRy(ii,jj,kk)=u1(ii,jj,kk)+0.5*slopey(ii,jj,kk)
  end do
 end do
end do

!------------------------------------------------------------------------------------
!Evolve Boundary Extrapolated values by a half-step: ubarL_i,j^(x,y), ubarR_i,j^(x,y)
!------------------------------------------------------------------------------------
do ii=0,nx+1
 do jj=0,ny+1 
  dens=max(qLx(ii,jj,1),min_dens)
  velx=qLx(ii,jj,2)/qLx(ii,jj,1)
  vely=qLx(ii,jj,3)/qLx(ii,jj,1)
  velz=qLx(ii,jj,4)/qLx(ii,jj,1)
  pres=max((gam-1.)*(qLx(ii,jj,5)-0.5*dens*(velx**2.+vely**2.+velz**2.)),min_pres)
  ...
  dens=max(qRx(ii,jj,1),min_dens)
  velx=qRx(ii,jj,2)/qRx(ii,jj,1)
  vely=qRx(ii,jj,3)/qRx(ii,jj,1)
  velz=qRx(ii,jj,4)/qRx(ii,jj,1)
  pres=max((gam-1.)*(qRx(ii,jj,5)-0.5*dens*(velx**2.+vely**2.+velz**2.)),min_pres)
  ...
  dens=max(qLy(ii,jj,1),min_dens)
  velx=qLy(ii,jj,2)/qLy(ii,jj,1)
  vely=qLy(ii,jj,3)/qLy(ii,jj,1)
  velz=qLy(ii,jj,4)/qLy(ii,jj,1)
  pres=max((gam-1.)*(qLy(ii,jj,5)-0.5*dens*(velx**2.+vely**2.+velz**2.)),min_pres)


  GL(1)=dens*vely 
  GL(2)=dens*velx*vely
  GL(3)=dens*vely*vely+pres
  GL(4)=dens*velz*vely
  GL(5)=0.5*dens*(velx*velx+vely*vely+velz*velz)*vely+gam*gamul*pres*vely


  dens=max(qRy(ii,jj,1),min_dens)
  velx=qRy(ii,jj,2)/qRy(ii,jj,1)
  vely=qRy(ii,jj,3)/qRy(ii,jj,1)
  velz=qRy(ii,jj,4)/qRy(ii,jj,1)
  pres=max((gam-1.)*(qRy(ii,jj,5)-0.5*dens*(velx**2.+vely**2.+velz**2.)),min_pres)

  GR(1)=dens*vely 
  GR(2)=dens*velx*vely
  GR(3)=dens*vely*vely+pres
  GR(4)=dens*velz*vely
  GR(5)=0.5*dens*(velx*velx+vely*vely+velz*velz)*vely+gam*gamul*pres*vely


  do kk=1,5
    ubLx(ii,jj,kk)=qLx(ii,jj,kk)+0.5*(dt/dx)*(FL(kk)-FR(kk)) &
                   +0.5*(dt/dy)*(GL(kk)-GR(kk))
    ubRx(ii,jj,kk)=qRx(ii,jj,kk)+0.5*(dt/dx)*(FL(kk)-FR(kk)) &
                   +0.5*(dt/dy)*(GL(kk)-GR(kk))
    ubLy(ii,jj,kk)=qLy(ii,jj,kk)+0.5*(dt/dx)*(FL(kk)-FR(kk)) &
                   +0.5*(dt/dy)*(GL(kk)-GR(kk))
    ubRy(ii,jj,kk)=qRy(ii,jj,kk)+0.5*(dt/dx)*(FL(kk)-FR(kk)) &
                   +0.5*(dt/dy)*(GL(kk)-GR(kk))
  end do
...
do ii=0,nx
 ...
 !x_interface
 rhoL=max(min_dens,ubRx(ii,jj,1))
 uL=ubRx(ii,jj,2)/ubRx(ii,jj,1)
 vL=ubRx(ii,jj,3)/ubRx(ii,jj,1)
 wL=ubRx(ii,jj,4)/ubRx(ii,jj,1)
 pL=max((gam-1.)*(ubRx(ii,jj,5)-0.5*rhoL*(uL*uL+vL*vL+wL*wL)),min_pres)
 aL=sqrt(gam*pL/rhoL)
 HL=0.5*(uL*uL+vL*vL+wL*wL)+gamul*aL*aL

 rhoR=max(min_dens,ubLx(ii+1,jj,1))
 uR=ubLx(ii+1,jj,2)/ubLx(ii+1,jj,1)
 vR=ubLx(ii+1,jj,3)/ubLx(ii+1,jj,1)
 wR=ubLx(ii+1,jj,4)/ubLx(ii+1,jj,1)
 pR=max((gam-1.)*(ubLx(ii+1,jj,5)-0.5*rhoR*(uR*uR+vR*vR+wR*wR)),min_pres)
 aR=sqrt(gam*pR/rhoR)
 ...
 !y-interface
 rhoL=max(min_dens,ubRy(ii,jj,1))
 uL=ubRy(ii,jj,3)/ubRy(ii,jj,1)
 vL=ubRy(ii,jj,2)/ubRy(ii,jj,1)
 wL=ubRy(ii,jj,4)/ubRy(ii,jj,1)
 pL=max((gam-1.)*(ubRy(ii,jj,5)-0.5*rhoL*(uL*uL+vL*vL+wL*wL)),min_pres)
 aL=sqrt(gam*pL/rhoL)
 HL=0.5*(uL*uL+vL*vL+wL*wL)+gamul*aL*aL

 rhoR=max(min_dens,ubLy(ii,jj+1,1))
 uR=ubLy(ii,jj+1,3)/ubLy(ii,jj+1,1)
 vR=ubLy(ii,jj+1,2)/ubLy(ii,jj+1,1)
 wR=ubLy(ii,jj+1,4)/ubLy(ii,jj+1,1)
 pR=max((gam-1.)*(ubLy(ii,jj+1,5)-0.5*rhoR*(uR*uR+vR*vR+wR*wR)),min_pres)
 aR=sqrt(gam*pR/rhoR)
 HR=0.5*(uR*uR+vR*vR+wR*wR)+gamul*aR*aR
 ...
 if(lambda1L<0. .and. lambda1R>0.)then
  ...
  g(ii,jj,1)=GL(1)+(lambda1L*(lambda1R-lambda(1)) &
               /(lambda1R-lambda1L))*alpha(1)*K1(1)
  g(ii,jj,2)=GL(3)+(lambda1L*(lambda1R-lambda(1)) &
               /(lambda1R-lambda1L))*alpha(1)*K1(3)
  g(ii,jj,3)=GL(2)+(lambda1L*(lambda1R-lambda(1)) &
               /(lambda1R-lambda1L))*alpha(1)*K1(2)
  g(ii,jj,4)=GL(4)+(lambda1L*(lambda1R-lambda(1)) &
               /(lambda1R-lambda1L))*alpha(1)*K1(4)
  g(ii,jj,5)=GL(5)+(lambda1L*(lambda1R-lambda(1)) &
               /(lambda1R-lambda1L))*alpha(1)*K1(5)

 else if(lambda5L<0. .and. lambda5R>0.)then

  g(ii,jj,1)=GR(1)-(lambda5R*(lambda(5)&
       -lambda5L)/(lambda5R-lambda5L))*alpha(5)*K5(1)
  g(ii,jj,3)=GR(3)-(lambda5R*(lambda(5)&
       -lambda5L)/(lambda5R-lambda5L))*alpha(5)*K5(2)
  g(ii,jj,2)=GR(2)-(lambda5R*(lambda(5)&
       -lambda5L)/(lambda5R-lambda5L))*alpha(5)*K5(3)
  g(ii,jj,4)=GR(4)-(lambda5R*(lambda(5)&
       -lambda5L)/(lambda5R-lambda5L))*alpha(5)*K5(4)
  g(ii,jj,5)=GR(5)-(lambda5R*(lambda(5)&
       -lambda5L)/(lambda5R-lambda5L))*alpha(5)*K5(5)

  
 else  
  g(ii,jj,1)=0.5*(GL(1)+GR(1))&
               -0.5*( alpha(1)*abs(lambda(1))*K1(1)&
               +alpha(2)*abs(lambda(2))*K2(1)&
     	       +alpha(3)*abs(lambda(3))*K3(1)&
     	       +alpha(4)*abs(lambda(4))*K4(1)&
     	       +alpha(5)*abs(lambda(5))*K5(1) )
  g(ii,jj,3)=0.5*(GL(3)+GR(3))&
               -0.5*( alpha(1)*abs(lambda(1))*K1(3)&
               +alpha(2)*abs(lambda(2))*K2(3)&
     	       +alpha(3)*abs(lambda(3))*K3(3)&
     	       +alpha(4)*abs(lambda(4))*K4(3)&
     	       +alpha(5)*abs(lambda(5))*K5(3) )
  g(ii,jj,2)=0.5*(GL(2)+GR(2))&
               -0.5*( alpha(1)*abs(lambda(1))*K1(2)&
               +alpha(2)*abs(lambda(2))*K2(2)&
     	       +alpha(3)*abs(lambda(3))*K3(2)&
     	       +alpha(4)*abs(lambda(4))*K4(2)&
     	       +alpha(5)*abs(lambda(5))*K5(2) )
  g(ii,jj,4)=0.5*(GL(4)+GR(4))&
               -0.5*( alpha(1)*abs(lambda(1))*K1(4)&
               +alpha(2)*abs(lambda(2))*K2(4)&
     	       +alpha(3)*abs(lambda(3))*K3(4)&
     	       +alpha(4)*abs(lambda(4))*K4(4)&
     	       +alpha(5)*abs(lambda(5))*K5(4) )
  g(ii,jj,5)=0.5*(GL(5)+GR(5))&
               -0.5*( alpha(1)*abs(lambda(1))*K1(5)&
               +alpha(2)*abs(lambda(2))*K2(5)&
     	       +alpha(3)*abs(lambda(3))*K3(5)&
     	       +alpha(4)*abs(lambda(4))*K4(5)&
     	       +alpha(5)*abs(lambda(5))*K5(5) )
 end if
