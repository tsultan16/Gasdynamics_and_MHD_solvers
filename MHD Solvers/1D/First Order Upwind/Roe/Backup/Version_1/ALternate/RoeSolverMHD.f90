...
real*8::BBR,BBL,vvR,vvL,gf1,gf7,gs3,gs5,theta1,theta2,vsq,ptotL,ptotR,ptot
real*8::vxt(7),uxt(7),phi
real*8::VLx(7),VRx(7),DUDV(7,7)
real*8::Rx1(7),Rx2(7),Rx3(7),Rx4(7),Rx5(7),Rx6(7),Rx7(7)
...
!-----------------------------------------------------
!Rx are the right eigenvectors for conserved variables
!-----------------------------------------------------
Rx1=0.
Rx2=0.
Rx3=0.
...
do k=1,7
Rx1(k)=Rx1(k)+DUDV(1,k)*R1(k)
Rx2(k)=Rx2(k)+DUDV(2,k)*R2(k)
Rx3(k)=Rx3(k)+DUDV(3,k)*R3(k)
Rx4(k)=Rx4(k)+DUDV(4,k)*R4(k)
Rx5(k)=Rx5(k)+DUDV(5,k)*R5(k)
Rx6(k)=Rx6(k)+DUDV(6,k)*R6(k)
Rx7(k)=Rx7(k)+DUDV(7,k)*R7(k)
end do
...
flux(j,k)=0.5*(FL(K)+FR(K))-0.5*( &
         abs(lambda(1))*alpha(1)*Rx1(k) &
        +abs(lambda(2))*alpha(2)*Rx2(k) &
        +abs(lambda(3))*alpha(3)*Rx3(k) &  
        +abs(lambda(4))*alpha(4)*Rx4(k) &
        +abs(lambda(5))*alpha(5)*Rx5(k) &
        +abs(lambda(6))*alpha(6)*Rx6(k) &
        +abs(lambda(7))*alpha(7)*Rx7(k) )
