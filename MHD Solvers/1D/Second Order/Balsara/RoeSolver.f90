...
!------------------------------------------------------------------------------
real*8::wL(7),wR(7),hL,hR
real*8::BBR,BBL,vvR,vvL,vsq
integer::j,k
!------------------------------------------------------------------------------
...
pL=(gam-1.)*(u1(j,7)-0.5*rhoL*vvL-0.5*BBL/(4.*pi))
ptotL=pL+0.5*BBL/(4.*pi)
hL=(u1(j,7)+pL+0.5*BBL/(4.*pi))/rhoL
...
pR=(gam-1.)*(u1(j+1,7)-0.5*rhoR*vvR-0.5*BBR/(4.*pi))
ptotR=pR+0.5*BBR/(4.*pi)
hR=(u1(j+1,7)+pR+0.5*BBR/(4.*pi))/rhoR

!------------------------------------------------------
!Construct parameter vectors for left and right states
!------------------------------------------------------
wL(1)=sqrt(rhoL)
wL(2)=sqrt(rhoL)*uL
wL(3)=sqrt(rhoL)*vL
...
!-------------------------------------------------------
!Roe-Averaged State Variables (density weighted average):
!-------------------------------------------------------
rhot=sqrt(rhoL*rhoR)
