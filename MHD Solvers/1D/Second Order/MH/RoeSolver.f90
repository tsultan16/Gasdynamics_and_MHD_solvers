...
real*8::alpha(-1:nx+2,7),lambda(-1:nx+2,7),Fleft(-1:nx+2,7),Fright(-1:nx+2,7)
...
BBL=bxLR*bxLR+byL*byL+bzL*bzL
vvL=uL*uL+vL*vL+wL*wL
pL=(gam-1.)*(qL(j,7)-0.5*rhoL*vvL-0.5*BBL)
...
BBR=bxLR*bxLR+byR*byR+bzR*bzR
vvR=uR*uR+vR*vR+wR*wR
pR=(gam-1.)*(qR(j,7)-0.5*rhoR*vvR-0.5*BBR)
...
!Enforce pressure positivity during reconstruction
!Set slope to zero of reconstructed p<0
!if(pL<tol .or. pR<tol)then
!  do k=1,7
!   ubL(j,k)=u1(j,k)
!   ubR(j,k)=u1(j+1,k)
!  end do
...
BBL=bxLR*bxLR+byL*byL+bzL*bzL
vvL=uL*uL+vL*vL+wL*wL
pL=(gam-1.)*(ubR(j,7)-0.5*rhoL*vvL-0.5*BBL)
ptotL=pL+0.5*BBL
...
BBR=bxLR*bxLR+byR*byR+bzR*bzR
vvR=uR*uR+vR*vR+wR*wR
pR=(gam-1.)*(ubL(j+1,7)-0.5*rhoR*vvR-0.5*BBR)
ptotR=pR+0.5*BBR

Fleft(j,1)=rhoL*uL
Fleft(j,2)=rhoL*uL*uL+pL+0.5*(BBL-2.*bxLR*bxLR)
Fleft(j,3)=rhoL*uL*vL-bxLR*byL
Fleft(j,4)=rhoL*uL*wL-bxLR*bzL
Fleft(j,5)=uL*byL-vL*bxLR
Fleft(j,6)=uL*bzL-wL*bxLR
Fleft(j,7)=uL*(ubR(j,7)+pL+0.5*BBL) &
      -bxLR*(bxLR*uL+byL*vL+bzL*wL)

Fright(j,1)=rhoR*uR
Fright(j,2)=rhoR*uR*uR+pR+0.5*(BBR-2.*bxLR*bxLR)
Fright(j,3)=rhoR*uR*vR-bxLR*byR
Fright(j,4)=rhoR*uR*wR-bxLR*bzR
Fright(j,5)=uR*byR-vR*bxLR
Fright(j,6)=uR*bzR-wR*bxLR
Fright(j,7)=uR*(ubL(j+1,7)+pR+0.5*BBR) &
      -bxLR*(bxLR*uR+byR*vR+bzR*wR)
...
do j=0,nx
 do k=1,7
  flux(j,k)=0.5*(Fleft(j,k)+Fright(j,k))-0.5*(     &
...
function limiter(x,y) result(z)
  ...
  !b=1 : MINBEE/MINMOD LIMITER
  !b=2 : SUPERBEE LIMITER
   
  b=1._8

  if(y>0._8)then
    z=max(0._8,min(b*x,y),min(x,b*y))
  else 
    z=min(0._8,max(b*x,y),max(x,b*y))
  end if
...
function minmod(x,y) result(z)
  ...
  z=sgn(x)*max(0._8,min(abs(x),sgn(x)*y))
