...
real*8,parameter::bxLR=2./sqrt(4.*pi)
...
open(unit=11,file='output_MH_fluid.txt')
open(unit=12,file='output_MH_B.txt')
...
do j=-1,nx+2
  ...
  if(j<nx/2) then
    ...
    u1(j,7)=0.5*rhoL*(uL*uL+vL*vL+wL*wL) &
           +gamul*pL+0.5*(bxLR*bxLR+byL*byL+bzL*bzL)
    ...
    u1(j,7)=0.5*rhoR*(uR*uR+vR*vR+wR*wR) &
           +gamul*pR+0.5*(bxLR*bxLR+byR*byR+bzR*bzR)
...
! else
  alpha(1)=alpha(1)+L1(k)*(ubL(j+1,k)-ubR(j,k))  
  alpha(2)=alpha(2)+L2(k)*(ubL(j+1,k)-ubR(j,k))
  alpha(3)=alpha(3)+L3(k)*(ubL(j+1,k)-ubR(j,k))
  alpha(4)=alpha(4)+L4(k)*(ubL(j+1,k)-ubR(j,k)) 
  alpha(5)=alpha(5)+L5(k)*(ubL(j+1,k)-ubR(j,k))
  alpha(6)=alpha(6)+L6(k)*(ubL(j+1,k)-ubR(j,k))
  alpha(7)=alpha(7)+L7(k)*(ubL(j+1,k)-ubR(j,k))
! end if
end do
