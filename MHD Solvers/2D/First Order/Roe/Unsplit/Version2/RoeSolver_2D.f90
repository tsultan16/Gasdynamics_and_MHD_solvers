...
integer,parameter::boundaryType=4 !1:outflow 2:periodic in x, outflow in y, 3:reflecting, 4:periodic
...
do jj=-1,nx+2
 ...
 do kk=-1,ny+2
  ...
  if(perturbationType==1)then
    vL=1.25*(ymax-ymin)*sin(kz*x)*exp(-300.*(kk-ny/2)**2.)
  !Random Perturbation
  else if(perturbationType==2)then
    vL=1.25*(ymax-ymin)*rand(0)*exp(-300.*(kk-ny/2)**2.)
  end if
  vR=vL
  !byL=vL*0.0000
  !byR=vL*0.0000
...
if(abs(byt*byt)<tol .and. abs(bzt*bzt)<tol .and. abs(cA*cA-at*at)<tol)then
 ...
 alphas=sqrt(abs(cf*cf-at*at)/abs(cf*cf-cs*cs))
 alphaf=sqrt(abs(cf*cf-cA*cA)/abs(cf*cf-cs*cs)) 
end if
