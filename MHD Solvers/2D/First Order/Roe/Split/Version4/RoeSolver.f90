...
subroutine timeStep(u1,smax)
!Input Variables
real*8::u1(-1:nx+2,-1:ny+2,8),smax

!Local Variables
integer::ii,jj
real*8::rhoL,uL,vL,wL,pL,bxL,byL,bzL,ptotL,vvL,BBL !left state
real*8::rhoR,uR,vR,wR,pR,bxR,byR,bzR,ptotR,vvR,BBR !right state 
real*8::cs,cf,cA
real*8::rhot,ut,vt,wt,pt,bxt,byt,bzt,at,ptot
real*8::bx,by,bz,bsqr
...
do jj=1,ny
  ...
  print*,'Local RP Right State (rho,u,v,w,bx,by,bz,p) =',rhoR,uR,vR,wR,bxR,byR,bzR,pR
   ...
   pt=ptot-0.5*(bxt*bxt+byt*byt+bzt*bzt)
   at=sqrt(gam*pt/rhot)
   ...
   !find maximum speed
   smax=max(smax,abs(max(ut,vt,wt))+cf)
...
subroutine computeFlux(u1,flux,n,ind,dir)

!Input Variables
real*8::u1(-1:nx+2,-1:ny+2,8),flux(-1:nx+2,7)
integer::n,ind,dir
...
integer::j,k
!------------------------------------------------------------------------------
real*8::rhoL,uL,vL,wL,pL,bxL,byL,bzL,ptotL,FL(7),uleft(7) !left state
real*8::rhoR,uR,vR,wR,pR,bxR,byR,bzR,ptotR,FR(7),uright(7) !right state 
real*8::bxLR
real*8::cs,cf,cA
...
!--------------------------------
rhoL=u1(j,ind,1)
uL=u1(j,ind,2)/u1(j,ind,1)
vL=u1(j,ind,3)/u1(j,ind,1)
wL=u1(j,ind,4)/u1(j,ind,1)
bxL=u1(j,ind,5)
byL=u1(j,ind,6)
bzL=u1(j,ind,7)
BBL=bxL*bxL+byL*byL+bzL*bzL
vvL=uL*uL+vL*vL+wL*wL
pL=(gam-1.)*(u1(j,ind,8)-0.5*rhoL*vvL-0.5*BBL)
ptotL=pL+0.5*BBL
...
rhoR=u1(j+1,ind,1)
uR=u1(j+1,ind,2)/u1(j+1,ind,1)
vR=u1(j+1,ind,3)/u1(j+1,ind,1)
wR=u1(j+1,ind,4)/u1(j+1,ind,1)
bxR=u1(j+1,ind,5)
byR=u1(j+1,ind,6)
bzR=u1(j+1,ind,7)
BBR=bxR*bxR+byR*byR+bzR*bzR
vvR=uR*uR+vR*vR+wR*wR
pR=(gam-1.)*(u1(j+1,ind,8)-0.5*rhoR*vvR-0.5*BBR)
ptotR=pR+0.5*BBR

uright(1)=u1(j+1,ind,1)
uright(2)=u1(j+1,ind,2)
uright(3)=u1(j+1,ind,3)
uright(4)=u1(j+1,ind,4)
uright(5)=u1(j+1,ind,6)
uright(6)=u1(j+1,ind,7)
uright(7)=u1(j+1,ind,8)

!--------------------------------
else if(dir==2)then
!--------------------------------
rhoL=u1(ind,j,1)
uL=u1(ind,j,3)/u1(ind,j,1)
vL=u1(ind,j,2)/u1(ind,j,1)
wL=u1(ind,j,4)/u1(ind,j,1)
bxL=u1(ind,j,6)
byL=u1(ind,j,5)
bzL=u1(ind,j,7)
BBL=bxL*bxL+byL*byL+bzL*bzL
vvL=uL*uL+vL*vL+wL*wL
pL=(gam-1.)*(u1(ind,j,8)-0.5*rhoL*vvL-0.5*BBL)
ptotL=pL+0.5*BBL
...
rhoR=u1(ind,j+1,1)
uR=u1(ind,j+1,3)/u1(ind,j+1,1)
vR=u1(ind,j+1,2)/u1(ind,j+1,1)
wR=u1(ind,j+1,4)/u1(ind,j+1,1)
bxR=u1(ind,j+1,6)
byR=u1(ind,j+1,5)
bzR=u1(ind,j+1,7)
BBR=bxR*bxR+byR*byR+bzR*bzR
vvR=uR*uR+vR*vR+wR*wR
pR=(gam-1.)*(u1(ind,j+1,8)-0.5*rhoR*vvR-0.5*BBR)
ptotR=pR+0.5*BBR

uright(1)=u1(ind,j+1,1)
uright(2)=u1(ind,j+1,3)
uright(3)=u1(ind,j+1,2)
uright(4)=u1(ind,j+1,4)
uright(5)=u1(ind,j+1,5)
uright(6)=u1(ind,j+1,7)
uright(7)=u1(ind,j+1,8)
...
F(j+1,7)=uR*(uright(7)+pR+0.5*BBR) &
...
do k=1,7  
  alpha(j,1)=alpha(j,1)+L1(j,k)*(uright(k)-uleft(k))  
  alpha(j,2)=alpha(j,2)+L2(j,k)*(uright(k)-uleft(k)) 
  alpha(j,3)=alpha(j,3)+L3(j,k)*(uright(k)-uleft(k)) 
  alpha(j,4)=alpha(j,4)+L4(j,k)*(uright(k)-uleft(k)) 
  alpha(j,5)=alpha(j,5)+L5(j,k)*(uright(k)-uleft(k)) 
  alpha(j,6)=alpha(j,6)+L6(j,k)*(uright(k)-uleft(k)) 
  alpha(j,7)=alpha(j,7)+L7(j,k)*(uright(k)-uleft(k)) 
end do
