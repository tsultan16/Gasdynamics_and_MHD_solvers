...
integer,parameter::boundaryType=1 !1:outflow 2:periodic in x, outflow in y, 3:reflecting
...
!-------------------------------------------------
bx=sqrt(bxLR*bxLR/rhot)
by=sqrt(byt*byt/rhot)
bz=sqrt(bzt*bzt/rhot)
bsqr=(bxLR*bxLR+byt*byt+bzt*bzt)/rhot
...
cA=bx
cs=0.5_8*(at*at+bsqr-sqrt((at*at+bsqr)*(at*at+bsqr)-4._8*at*at*cA*cA))
cf=0.5_8*(at*at+bsqr+sqrt((at*at+bsqr)*(at*at+bsqr)-4._8*at*at*cA*cA))
cs=sqrt(cs)
cf=sqrt(cf)
