...
integer,parameter::k=3 !candidate stencil size (set k<=3)
integer,parameter::nt=300 !total time steps
integer,parameter::r_option=2 !1:ENO, 2:WENO
...
do i=0,nx+1
  do j=0,k-1
    vleft(i,j)=p(i-1,j,Vdiff,1)
    vright(i,j)=p(i,j,Vdiff,2)
  end do
end do
...
do i=0,nx+1
    vL(i)=p(i,r(i),Vdiff,1)
    vR(i)=p(i,r(i),Vdiff,2)
end do
